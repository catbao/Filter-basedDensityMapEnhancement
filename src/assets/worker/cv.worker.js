...
function mappingValueTo2DColorbar(input, result, {minVal, maxVal}) {
  ...
  input.map(row => row.map(val => {
    if (val < inputMin) { inputMin = val; }
    if (val > inputMax) { inputMax = val; }
  }));

  let coloredResult = input.map((row, i) => row.map((val,j) => {
    let hueIdx = Math.max(Math.ceil(minMaxScale(val, inputMin, inputMax, 0, current_2d_colormap.rows))-1, 0);
    let lightnessIdx = Math.max(Math.ceil(minMaxScale(result.floatAt(i, j), minVal, maxVal, 0, current_2d_colormap.cols))-1, 0);
    return Array.from(current_2d_colormap.ucharPtr(lightnessIdx, hueIdx));
  }));
  return cv.matFromArray(result.rows, result.cols, cv.CV_8UC4, coloredResult.flat(2));
}
 ...
 */
function enhanceDensityMap({ msg, densitymap, originalData, paramStr, regionLensStr }) {
  ...
  switch (params.filter) {
    case 'gaussian':
      let d = params.filterParams.win_size % 2 == 0 ? params.filterParams.win_size+1 : params.filterParams.win_size,
          sigma = params.filterParams.sigma;
      cv.GaussianBlur(src, baseLayer, new cv.Size(d,d), sigma, sigma, cv.BORDER_DEFAULT);
      break;
    case 'guided':
      guidedFilter(src, src, baseLayer, params.filterParams.win_size, params.filterParams.tau);
      break;
    default:
  ...
  postMessage({ msg, imgData: clampedArray, minVal: resultMinMaxLoc.minVal, maxVal: resultMinMaxLoc.maxVal })
...
function adjust2DColorMap({s, e, o, n}) {
    ...
    for(let i=0; i<4; ++i) {
      ...
      if(w > 0) {
        let piece = current_2d_colormap.roi(new cv.Rect(oldRange[i],0,w,current_2d_colormap.rows));
        if(i > 0 && i < 3) { // only resize for colors within [s,e]
          cv.resize(piece, piece, new cv.Size(newRange[i+1]-newRange[i], current_2d_colormap.rows))
        }
  ...
  switch (e.data.msg) {
    ...
    case 'adjusting2d':
      ...
      if(current_2d_colormap == undefined) {
        ...
        clampedArray = new ImageData(new Uint8ClampedArray(dst.data), dst.cols, dst.rows);
        dst.delete();
      }
